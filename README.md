...
npm install -D typescript@5.5.4 @types/node@20.14.10 @types/react@18.3.3 \
...
threshold. Only non-`BLOCK`ed signups get written into the store, so a
...
1. Load `/signup` — no signals collected yet.
2. Submit the form — `collectSignals()` runs in the browser, producing both
   fingerprints.
3. POST to `/api/signup` — server extracts the real IP from headers.
4. `evaluateSignup()` checks the store for fingerprint matches, disposable
...
5. Decision, score, and reasons flow back to the UI and render immediately.
...
The core principle that makes this "optimal" instead of "brute-force" is:
**no single signal is trusted as a hard gate.** Device fingerprints,
IP velocity, email reputation, and automation flags are each weak on their
own (each has known bypasses), but summing them into a score and responding
with tiered friction closes most of the gaps while keeping false positives
low for real users. This is the same shape used by every major production
fraud system reviewed in this project — only the number and sophistication
of signals changes at scale, not the fundamental architecture.